...
This repo also ships with a Next.js shell around the original canvas game. The game scripts
still load in the same order from `public/js` and `public/mods`, while Next.js provides the
page shell, Zustand app state, responsive fullscreen scaling, VI/EN language switching, and
MongoDB-backed cloud saves.

```bash
npm install
cp .env.example .env.local    # set MONGODB_URI if you want cloud saves
npm run dev                   # http://localhost:3000, or the next free port
npm run build
```

If `MONGODB_URI` is not configured, the game still runs and keeps using localStorage saves.
The save API is available at `/api/save/default`.
