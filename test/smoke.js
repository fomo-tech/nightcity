...
G.enemies = [allyBot];
const followSpot = findSpot(G.p.x, G.p.y, 110, 140);
assert(followSpot, 'player gang bot follow test spot exists');
allyBot.x = followSpot.x; allyBot.y = followSpot.y; allyBot.alerted = false; allyBot.wanderT = 0;
const followD0 = distPx(allyBot.x, allyBot.y, G.p.x, G.p.y);
updateEnemies(0.5);
assert(distPx(allyBot.x, allyBot.y, G.p.x, G.p.y) < followD0, 'player gang bot follows V when idle');
G.enemies = [];
const ambusher = makeEnemy(G.p.x + 70, G.p.y, 1, 'scavs', 'gun', { gangPack: true });
ambusher.lookA = 0; ambusher.wanderT = 999;
G.enemies.push(ambusher);
steps(6);
assert(ambusher.alerted && ambusher.lkx === G.p.x && ambusher.lky === G.p.y, 'hostile gang bot proactively locks player inside aggro range');
G.enemies = [];
